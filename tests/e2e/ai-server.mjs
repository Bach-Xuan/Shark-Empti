...
createServer(async (request, response) => {
  ...
  let output;
  if ('userMessage' in data) output = { aiResponse: explanation };
  ...
  else if ('concept' in data) output = { questions: Array.from({ length: data.numQuestions || 1 }, () => ({ ...question, options: ['4', '3'] })) };
